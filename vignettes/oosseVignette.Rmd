...
The _oosse_ function works with any pair of fitting and prediction functions. Here we illustrate a number of them, but any prediction function implemented in R can be used. The built-in dataset _Brassica_ is used, which contains _rlog_-transformed gene expression measurements for the 1,000 most expressed genes in the _Expr_ slot, as well as 5 outcome phenotypes in the _Pheno_ slot.
...
```{r LMpred}
R2lm <- oosse(
  y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, 1:5],
...
R2lm632jn <- oosse(
  y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, 1:5],
  fitFun = fitFunLM, predFun = predFunLM, methodLoss = "bootstrap",
  methodCor = "jackknife"
)
```

Supplying a dataframe with predictor variables is not allowed. The user is asked to build the design matrix yourself with model.matrix prior to calling _oosse_:
...
# Now run oosse
R2modMat <- oosse(
  y = Brassica$Pheno$Leaf_8_width, x = designMatrix,
  fitFun = fitFunLM, predFun = predFunLM
)
```
...
```{r reglinmod}
fitFunReg <- function(y, x, ...) {
  cv.glmnet(y = y, x = x, ...)
}
predFunReg <- function(mod, x, ...) {
  predict(mod, newx = x, ...)
}
```
...
if (require(glmnet)) {
  ...
  R2pen <- oosse(
    y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, seq_len(numFeat)], # Subset genes for speed
    nFolds = nFolds, cvReps = cvReps, nBootstrapsCor = nBoots,
    fitFun = fitFunReg, predFun = predFunReg, alpha = 1
  ) # Lasso model
  R2pen$R2
}
```
...
if (require(randomForest)) {
  ...
  fitFunrf <- function(y, x, ...) {
    randomForest(y = y, x, ...)
  }
  predFunrf <- function(mod, x, ...) {
    predict(mod, x, ...)
  }
  R2rf <- oosse(
    y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, seq_len(numFeat)],
    nFolds = nFolds, cvReps = cvReps, nBootstrapsCor = nBoots,
