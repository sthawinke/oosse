...
```{r LMpred}
R2lm = oosse(y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, 1:5], 
...
```{r lmBoot}
R2lm632jn = oosse(y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, 1:5], 
...
#Now run oosse
R2modMat = oosse(y = Brassica$Pheno$Leaf_8_width, x = designMatrix, 
                    fitFun = fitFunLM, predFun = predFunLM)
```
...
if(require(glmnet)){
    ...
    R2pen = oosse(y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, seq_len(numFeat)], #Subset genes for speed
...
if(require(randomForest)){
   if(onWindows){
    ...
    R2rf = oosse(y = Brassica$Pheno$Leaf_8_width, x = Brassica$Expr[, seq_len(numFeat)],
                     nFolds = nFolds, cvReps = cvReps, nBootstrapsCor = nBoots,
