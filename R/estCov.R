...
#' the average outcome (\eqn{\bar{y}}) and the variance of \eqn{\hat{\kappa}} through the bootstrap.
#'
#' @inheritParams oosse
#'
#' @return The estimated covariance and variance
...
estCorMeanRef <- function(y, x, fitFun, predFun, methodLoss, methodCor, nBootstrapsCor,
    ...
    nReps <- switch(methodCor,
        "nonparametric" = nBootstrapsCor,
        "jackknife" = length(y)
    )
    matMSEMST <- simplify2array(bplapply(seq_len(nReps), function(i) {
        id <- switch(methodCor,
            "nonparametric" = sample(length(y), replace = TRUE),
            "jackknife" = -i
        )
        modelLoss <- switch(methodLoss,
            "bootstrap" = boot632multiple(
                nBootstraps = nBootstraps, y[id], x[id, , drop = FALSE], yMat = yMat[id, , drop = FALSE],
                fitFun, predFun, loss = loss, skillScore = skillScore
            ),
            "CV" = simpleCV(y[id], x[id, , drop = FALSE],
                yMat = yMat[id, , drop = FALSE],
                fitFun, predFun, nFolds, loss = loss, skillScore = skillScore
            )
        )
        refLoss <- estRefLoss(y[id], x[id, , drop = FALSE], skillScore,
            kappaHat = modelLoss["kappaHat"],
            nBootstraps = nBootstraps, fitFun = fitFun, predFun = predFun
        )[
