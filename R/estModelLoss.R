...
estModelLoss <- function(y, x, fitFun, predFun, methodLoss, nFolds, nInnerFolds,
    ...
    seVec <- if (methodLoss == "CV") {
        ...
        unFolds <- seq_len(nFolds)
        unFoldsIn <- seq_len(nInnerFolds) # Prepare the folds
        cvSplitReps <- bplapply(seq_len(cvReps), function(cvs) {
            ...
            lapply(unFolds, function(uf) {
                ...
                list(
                    "a" = a, "b" = b, "errHatTilde" = errHatTilde, "eOut" = eOut,
                    "predTest" = predTest
                )
            })
        })
        ...
        MSE632est <- mean(vapply(FUN.VALUE = double(1), bootReps, function(x) {
            x$MSE632est
        }), na.rm = TRUE)
