...
test_that("confidence intervals are built with correct boundaries", {
    ...
    expect_silent(confIntPeirce <- buildConfInt(HeidkeObj))
    expect_silent(confIntModelHSS <- buildConfInt(HeidkeObj, what = "ModelMisclassRate"))
    expect_silent(confIntRefHSS <- buildConfInt(HeidkeObj, what = "ReferenceMisclassRate", conf = 0.94))
