...
Peirceobj <- oosse(y = yBin, x = x, predFun = predFunBin, fitFun = fitFunBin, printTimeEstimate = FALSE, skillScore = "Heidke")
...
test_that("confidence intervals are built with correct boundaries", {
  ...
  expect_silent(confIntPeirce <- buildConfInt(Peirceobj))
  expect_silent(confIntModelHSS <- buildConfInt(Peirceobj, what = "ModelMisclassRate"))
  expect_silent(confIntRefHSS <- buildConfInt(Peirceobj, what = "ReferenceMisclassRate", conf = 0.94))
  expect_true(all(confIntPeirce <= 1))
  ...
  expect_silent(confIntRefMSS <- buildConfInt(MSSobj, what = "ReferenceMisclassRate", conf = 0.94))
...
test_that("buildConfInt throws an error when incorrect input is provided", {
  ...
  expect_error(buildConfInt(Peirceobj, what = "MSE"))
})
